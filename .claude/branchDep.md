...
| 2026-09-12 | extension/package.json (new file) | + `esbuild@^0.28.0`, `typescript@^5.0.0`, `vitest@^5.0.0`, `@types/chrome@^0.0.280`, `jsdom@^27.0.0` | Phase 14 browser extension — build/typecheck/test tooling for the new `extension/` package. Not yet covered by `instructions.md`'s Dependency Rule (which names only backend/frontend files) — logged here anyway in the rule's spirit; pending user decision on whether to extend the rule's file list. |
