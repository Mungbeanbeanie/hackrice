...
- [x] `frontend/src/components/SpecBreakdownModal.tsx` — remove `equivalent` from `VERDICT_STYLE`; `SpecRow.verdict` becomes `Verdict | null` throughout; render the verdict chip only when non-null; guard optional `rationale`
- [x] `backend/tests/test_explain.py` (new) — same/close/better/lower/different cases, `None` when the target lacks the spec, union-coverage test asserting all 5 verdict values are reachable, rationale savings-clause present/absent, assert no Q/V figures leak into rationale text
