...
- [ ] `backend/app/ingestion/serpapi_client.py` — add `_MATERIAL` regex (same shape as `_SIZE`, line 40), closed cross-category vocabulary (`memory foam` before `foam` — first-match alternation, not longest); ships a `material` spec only, `weight_tier="hard"`, no `feature_*` booleans
- [ ] `backend/app/models.py` — `Tier` → `Group` StrEnum (`SAME_SPEC`/`SAME_JOB`/`CLEARS_FLOOR`); `ComparisonResult.tier` → `.group`. No `direction` field (see cache-safety note above).
- [ ] `backend/app/scoring/value.py` — `_assign_tier` → `_assign_group` (same thresholds/logic); add explicit `_GROUP_RANK` dict and fix the sort key (line 86) — the new group names sort `clears_floor < same_job < same_spec` lexicographically, an exact reversal of the old `tier_1 < tier_2 < tier_3` ordering that the sort currently relies on; needs a regression test, since the current `test_scoring.py` asserts a set, not order
- [ ] `backend/app/config.py` — rename `SPEC_MATCH_TIER1`/`SPEC_MATCH_TIER2` → `SPEC_MATCH_SAME_SPEC`/`SPEC_MATCH_SAME_JOB` (confirmed safe — no compose/CI/`.env` override exists anywhere in the repo)
